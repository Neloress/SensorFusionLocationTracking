...
namespace SensorFusionLocationTracking
{
	internal class Matrix
	{
		...
		internal static Matrix GetRotationAroundAxis(double rad, Vector vector)
		{
			...
			Matrix m = new Matrix();
			m.V[0, 0] = v.X * v.X * (1.0 - Math.Cos(rad)) + Math.Cos(rad);
			m.V[1, 0] = v.X * v.Y * (1.0 - Math.Cos(rad)) - v.Z * Math.Sin(rad);
			m.V[2, 0] = v.X * v.Z * (1.0 - Math.Cos(rad)) + v.Y * Math.Sin(rad);
			m.V[0, 1] = v.Y * v.X * (1.0 - Math.Cos(rad)) + v.Z * Math.Sin(rad);
			m.V[1, 1] = v.Y * v.Y * (1.0 - Math.Cos(rad)) + Math.Cos(rad);
			m.V[2, 1] = v.Y * v.Z * (1.0 - Math.Cos(rad)) - v.X * Math.Sin(rad);
			m.V[0, 2] = v.Z * v.X * (1.0 - Math.Cos(rad)) - v.Y * Math.Sin(rad);
			m.V[1, 2] = v.Z * v.Z * (1.0 - Math.Cos(rad)) + v.X * Math.Sin(rad);
			m.V[2, 2] = v.Z * v.Z * (1.0 - Math.Cos(rad)) + Math.Cos(rad);
